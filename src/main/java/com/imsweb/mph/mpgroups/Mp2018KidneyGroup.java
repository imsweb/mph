...
public class Mp2018KidneyGroup extends MphGroup {
    ...
    Rule M9	Abstract multiple primaries when separate/non-contiguous tumors are on different rows in Table 1 in the Equivalent Terms and Definitions. Tumors must be in the same kidney.
    ...
    */
    ...
    public Mp2018KidneyGroup() {
        ...
        // Rule M6	Abstract multiple primaries when the patient has a subsequent tumor after being clinically disease-free for greater than three years after the original diagnosis or last recurrence.
        rule = new MphRuleDiagnosisDateGreaterThan3Years(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M6");
        ...
        rule = new MphRuleTwoOrMoreDifferentSubTypesInTable(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M7", MphConstants.KIDNEY_2018_TABLE1_SUBTYPES, MphConstants.KIDNEY_2018_SUBTYPE_NOS, true);
        ...
        // Rule M8	Abstract a single primary when synchronous, separate/non-contiguous tumors are on the same row in Table 1 in the Equivalent Terms and Definitions. Tumors must be in the same kidney.
        rule = new MphRuleSameRowInTable(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M8", MphConstants.KIDNEY_2018_TABLE1_ROWS, MphConstants.KIDNEY_2018_SUBTYPE_NOS, true, true, false);
        ...
        rule = new MphRuleDifferentRowsInTable(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M9", MphConstants.KIDNEY_2018_TABLE1_ROWS, true);
        ...
        // Rule M10	Abstract a single primary when an in situ tumor is diagnosed after an invasive tumor AND tumors occur in the same kidney.
        rule = new MphRuleInSituAfterInvasive(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M10", true, false);
        ...
        // Rule M11	Abstract a single primary (the invasive) when an invasive tumor is diagnosed less than or equal to 60 days after an in situ tumor in the same kidney.
        rule = new MphRuleInvasiveAfterInSituLess60Days(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M11", true, false);
        rule.getNotes().add("The rules are hierarchical. Only use this rule when none of the previous rules apply.");
        ...
        // Rule M12	Abstract multiple primaries when an invasive tumor occurs more than 60 days after an in situ tumor in the same kidney.
        rule = new MphRuleInvasiveAfterInSituGreaterThan60Days(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M12", true);
        rule.getNotes().add("The rules are hierarchical. Only use this rule when none of the previous rules apply.");
        ...
        // Rule M13	Abstract a single primary when there are multiple tumors that do not meet any of the above criteria.
        rule = new MphRuleNoCriteriaSatisfied(MphConstants.MP_2018_KIDNEY_GROUP_ID, "M13");
