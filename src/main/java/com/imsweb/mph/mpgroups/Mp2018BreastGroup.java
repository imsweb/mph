...
public class Mp2018BreastGroup extends MphGroup {
    ...
    Rule M5	Abstract multiple primaries when the patient has a subsequent tumor after being clinically disease-free for greater than five years after the original diagnosis or last recurrence.
    ...
    */
    ...
    public Mp2018BreastGroup() {
        ...
        // Rule M7	Abstract multiple primaries when there is bilateral breast cancer (both right and left breast).
        rule = new MphRuleLeftAndRight(MphConstants.MP_2018_BREAST_GROUP_ID, "M7", null, null);
        ...
        // Rule M11	Abstract multiple primaries when separate/non-contiguous tumors are two or more different subtypes/variants in Column 3 of Table 3 in the Equivalent Terms and Definitions. Timing is irrelevant.
        rule = new MphRuleTwoOrMoreDifferentSubTypesInTable(MphConstants.MP_2018_BREAST_GROUP_ID, "M11", MphConstants.BREAST_2018_TABLE3_SUBTYPES, MphConstants.BREAST_2018_SUBTYPE_NOS, false);
        ...
        // Rule M14	Abstract a single primary (the invasive) when an in situ tumor is diagnosed after an invasive tumor in the same breast.
        rule = new MphRuleInSituAfterInvasive(MphConstants.MP_2018_BREAST_GROUP_ID, "M14", true, false);
        ...
        // Rule M15	Abstract a single primary (the invasive) when an invasive tumor is diagnosed less than or equal to 60 days after an in situ tumor in the same breast.
        rule = new MphRuleInvasiveAfterInSituLess60Days(MphConstants.MP_2018_BREAST_GROUP_ID, "M15", true, false);
        rule.getNotes().add("The rules are hierarchical. Only use this rule when none of the previous rules apply.");
        rule.getNotes().add("The tumors may be a NOS and a subtype/variant of that NOS.");
