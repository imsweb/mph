...
public class Mp2018LungGroup extends MphGroup {
    ...
    Rule M4     Abstract multiple primaries when the patient has a subsequent tumor after being clinically disease-free for greater than three years after the original diagnosis.
                Note 1: Clinically disease-free means that there was no evidence of recurrence on follow-up in the same lung.
    ...
    Rule M5     Abstract multiple primaries when separate/non-contiguous tumors are on different rows in Table 3, first or second column, in the Equivalent Terms and Definitions. Tumors may be simultaneous or original and subsequent.
    ...
    Rule M6     Abstract multiple primaries when separate/non-contiguous tumors are different subtypes/variants in Column 3, Table 3 in the Equivalent Terms and Definitions. Tumors may be simultaneous or original and subsequent.
    ...
    Rule M7     Abstract a single primary when there are multiple tumors in the same lung that are an NOS and a subtype/variant of that NOS.
                ...
                • Adenocarcinoma NOS 8140 and any subtype/variant of adenocarcinoma
                • Any two subtypes of non-small cell carcinoma
                • Large cell carcinoma NOS 8012 and any subtype/variant of large cell carcinoma
                • Malignant neoplasm NOS 8000 or malignant tumor cells, NOS 8001 and any of the subtypes/variants
                • Mucinous adenocarcinoma, invasive 8253/3 and any subtype/variant of invasive mucinous adenocarcinoma
                • Non-mucinous adenocarcinoma, invasive 8140 and any subtype/variant of non-mucinous adenocarcinoma
                • Non-small cell carcinoma 8046 and any subtype/variant of non-small cell
                • Pleomorphic carcinoma 8022 and any subtype/variant of pleomorphic carcinoma
                • Sarcomatoid carcinoma NOS and any subtype/variant of sarcomatoid carcinoma
                • Small cell neuroendocrine tumors/NET 8041 and any subtype/variant of small cell neuroendocrine tumor/NET
                • Squamous cell carcinoma NOS 8070 and any subtype/variant of squamous cell carcinoma
                • Synovial sarcoma 9040 and any subtype/variant of synovial sarcoma

    Rule M8     Abstract a single primary when there are simultaneous in situ and invasive tumors in the same lung.
                Example: One tumor is invasive adenocarcinoma 8140/3 AND another tumor is adenocarcinoma in situ 8140/2.

    Rule M9     Abstract a single primary when an in situ tumor is diagnosed after an invasive tumor AND tumors occur in the same lung.
                Note 1: The rules are hierarchical. Do not use this rule if any of the previous rules apply.
    ...
    Rule M10    Abstract multiple primaries when there is a single tumor in each lung (one tumor in the right lung and one tumor in the left lung).
                ...
                • Attending physician, oncologist, or pulmonologist state unequivocally that the tumor in the contralateral lung is metastatic
                    ο Unequivocal means that no words such as “probably possibly, most likely, etc.” are used in the statement. This statement purposefully contains terms which are not on the “ambiguous terms” list to demonstrate that a number of words can make the statement equivocal (cannot be used to prove metastases)
                ...
                Note 4: This rule is based on long-term epidemiologic studies of multiple primaries. The specialty medical experts (SME) and the CoC site physician teams reviewed and approved these rules. Many of the CoC site team physicians were also authors, coauthors, or editors of the AJCC Staging Manual.

    Rule M11    Abstract multiple primaries when there is at least one tumor that is small cell carcinoma 8041 or any small cell subtypes/variants and another tumor that is non-small cell carcinoma 8046 or any of the non-small cell carcinoma subtypes/variants.
    ...
    Rule M12    Abstract a single primary when there are simultaneous:
                • A single tumor in one lung and multiple tumors in the contralateral lung OR
                • Multiple tumors in both lungs OR
                • Multiple tumors in the same lung
    ...
    Rule M13    Abstract a single primary (the invasive) when an invasive tumor is diagnosed less than or equal to 60 days after an in situ tumor in the same lung.
    ...
    Rule M14    Abstract multiple primaries when an invasive tumor occurs more than 60 days after an in situ tumor in the same lung.
    ...
    Rule M15    Abstract multiple primaries when there are tumors with ICD-O histology codes that are different at the first (Xxxx), second (xXxx), or third (xxXx) digit.
                Note: Rules are hierarchical. Previous rules covered laterality (tumors in both lungs), so this rule would only apply to tumors in the same lung. A previous rule covered NOS and a subtype/variant of that NOS, so this rule would not apply to a NOS and a subtype/variant of that NOS.
                Example: Acinar adenocarcinoma 8551 in left upper lobe and squamous cell carcinoma 8070 in left lower lobe are multiple primaries.
    ...
    public Mp2018LungGroup() {
        ...
        rule.getNotes().add("Clinically disease-free means that there was no evidence of recurrence on follow-up in the same lung.");
        ...
        rule.getNotes().add("• Adenocarcinoma NOS 8140 and any subtype/variant of adenocarcinoma");
        rule.getNotes().add("• Any two subtypes of non-small cell carcinoma");
        rule.getNotes().add("• Large cell carcinoma NOS 8012 and any subtype/variant of large cell carcinoma");
        rule.getNotes().add("• Malignant neoplasm NOS 8000 or malignant tumor cells, NOS 8001 and any of the subtypes/variants");
        rule.getNotes().add("• Mucinous adenocarcinoma, invasive 8253/3 and any subtype/variant of invasive mucinous adenocarcinoma");
        rule.getNotes().add("• Non-mucinous adenocarcinoma, invasive 8140 and any subtype/variant of non-mucinous adenocarcinoma");
        rule.getNotes().add("• Non-small cell carcinoma 8046 and any subtype/variant of non-small cell");
        rule.getNotes().add("• Pleomorphic carcinoma 8022 and any subtype/variant of pleomorphic carcinoma");
        rule.getNotes().add("• Sarcomatoid carcinoma NOS and any subtype/variant of sarcomatoid carcinoma");
        ...
        rule.getNotes().add("• Squamous cell carcinoma NOS 8070 and any subtype/variant of squamous cell carcinoma");
        ...
        rule.setQuestion("");
        rule.setReason("");
        rule.getNotes().add("The rules are hierarchical. Do not use this rule if any of the previous rules apply.");
        ...
        rule.getNotes().add("  ο Unequivocal means that no words such as “probably possibly, most likely, etc.” are used in the statement. This statement purposefully contains terms which are not on the “ambiguous terms” list to demonstrate that a number of words can make the statement equivocal (cannot be used to prove metastases)");
        ...
        rule.getNotes().add("This rule is based on long-term epidemiologic studies of multiple primaries. The specialty medical experts (SME) and the CoC site physician teams reviewed and approved these rules. Many of the CoC site team physicians were also authors, coauthors, or editors of the AJCC Staging Manual.");
        _rules.add(rule);
        ...
        rule.setQuestion("");
        rule.setReason("");
        rule.getNotes().add("Note 1: When there are multiple tumors in one or both lungs, the physician usually biopsies only one mass/tumor. They treat the patient based on that single biopsy, assuming all of the masses/tumors are the same histology.");
        rule.getNotes().add("Note 2: Code multiple primaries only when there is proof that one of the tumors is a different histology. Proof is any one of the following.");
        ...
        // Rule M15 - Abstract multiple primaries when there are tumors with ICD-O histology codes that are different at the first (Xxxx), second (xXxx), or third (xxXx) digit.
        rule = new MphRuleHistologyCode(MphConstants.MP_2018_LUNG_GROUP_ID, "M15");
        rule.getNotes().add("Rules are hierarchical. Previous rules covered laterality (tumors in both lungs), so this rule would only apply to tumors in the same lung. A previous rule covered NOS and a subtype/variant of that NOS, so this rule would not apply to a NOS and a subtype/variant of that NOS.");
        rule.getExamples().add("Acinar adenocarcinoma 8551 in left upper lobe and squamous cell carcinoma 8070 in left lower lobe are multiple primaries.");
        _rules.add(rule);
